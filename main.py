...
def render_colorpicker():
    ...
    for x in range(255):
        for y in range(255):
            ...
            pygame.draw.rect(screen, (r, g, b), (shiftx + x, shifty + y, 1, 1))
            if (x, y, z_choose) == cp_color:
                cpcx, cpcy = shiftx + x, shifty + y

    pygame.draw.rect(screen, (0, 0, 0), (mouse_x - 5, mouse_y - 5, 10, 10), 1)
    pygame.draw.rect(screen, (255, 255, 255), (mouse_x - 4, mouse_y - 4, 8, 8), 1)

    for z in range(255):
        r, g, b = [int(c * 255) for c in colorsys.hsv_to_rgb(z / 255.0, 1.0, 1.0)]
        pygame.draw.rect(screen, (r, g, b), (shiftx + z, 325, 1, 20))
...
None, None, None, None, None, (63, 140, 255), (255, 66, 122), (44, 255, 102), None)
shiftx, shifty = width // 2 + 60, 50
can, cp_open, cp_color, draw, message, cpcx, cpcy, z_choose, cp_rect, cp_crect, cp_zrect = 1, 0, (
255, 255, 255), 0, 'Text...', shiftx + 255, shifty + 255, 0, pygame.Rect(width // 2 + 40, 30, 295, 355), pygame.Rect(
    width // 2 + 60, 50, 255, 255), pygame.Rect(width // 2 + 60, 325, 255, 20)
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if clicked:
                if cp_open:
                    if cp_crect.collidepoint(mouse_x, mouse_y):
                        s = (mouse_x - shiftx) / 255.0
                        v = 1 - ((mouse_y - shifty) / 255.0)
                        h = z_choose / 255.0
                        r, g, b = [int(c * 255) for c in colorsys.hsv_to_rgb(h, s, v)]
                        cp_color = color = (r, g, b)
                    elif cp_zrect.collidepoint(mouse_x, mouse_y):
                        z_choose = max(0, min(255, mouse_x - shiftx))
            ...
            if cp_open == 1:
                ...
                if not cp_rect.collidepoint(cpmx, cpmy):
                    ...
                    if cp_crect.collidepoint(cpmx, cpmy):
                        s = (cpmx - shiftx) / 255.0
                        v = 1 - ((cpmy - shifty) / 255.0)
                        h = z_choose / 255.0
                        r, g, b = [int(c * 255) for c in colorsys.hsv_to_rgb(h, s, v)]
                        cp_color = color = (r, g, b)
                    elif cp_zrect.collidepoint(cpmx, cpmy):
                        z_choose = max(0, min(255, cpmx - shiftx))
            else:
                if not mouse_y < 30:
                    draw = 1
                    coordinates.append({'ox': origin[0], 'oy': origin[1], 'x': mouse_x, 'y': mouse_y,
                                        'fill': fill, 'color': color, 'tool': current, 'message': message})
            ...
            elif pygame.key.name(event.key) in [chr(i) for i in range(33, 127)]:
    ...
    for item in coordinates:
        if item['tool'] == 1:
            ...
            abs(item['ox'] - item['x']), abs(item['oy'] - item['y'])), 0, 3 * 3.14, item['fill'])
        elif item['tool'] == 4:
            screen.blit(pygame.font.Font('font.ttc', 32).render(item['message'], False, item['color']),
                        (item['ox'], item['oy']))
